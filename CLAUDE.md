...
- [src/lib/otp.ts](src/lib/otp.ts) + [src/lib/otp-clerk.ts](src/lib/otp-clerk.ts) — WhatsApp OTP login. `/api/auth/otp/request` sends the code; `/api/auth/otp/verify` verifies it, then `mintClerkSignInToken()` find-or-creates a Clerk user by E.164 phone and returns a 60s sign-in token the client redeems via Clerk's `strategy: "ticket"`. Requires phone login enabled on the Clerk instance.
